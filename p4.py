...
def spacecraft_triliteration(T, measured_distances):
    ...
    Returns estimated x and y pos of rocket calculated from triliteration-method.
    """
    # Finding the closest idx in the time-array of the desired launchtime:
    time_diff = orbit_0[0] - T
    ...
    theta_array = np.arange(
        0, 2 * np.pi, 10e-7
    )  # Creates an array of angles from 0 to 2*pi to parametrize circles.
    ...
    search_2 = np.where(np.abs((abs_diff_2 - star_distance)) < 10e-7)[
        0
    ]  # narrowing down possible idx-points on circle 2 using a tolerance
    search_5 = np.where((np.abs(abs_diff_5 - star_distance)) < 10e-7)[
        0
    ]  # narrowing down possible idx-points on circle 5 uring a tolerance
    n = len(search_2)
    m = len(search_5)
    matching_position = []
    ...
    least_diff = 100  # setting a big initial difference to be used to find match.
    most_accurate_idx = 0  # setting the initial idx of best match
...
def test_equipment(phi, T):
    ...
    least_time_diff = np.min(time_diff)
    idx = np.where(time_diff == least_time_diff)[0]
    initial_x_pos = orbit_0[1][idx] + (
        ((homeplanet_radius / Au) * np.cos((np.pi / 2) - np.pi / 2) * np.cos(phi))
    )  # starting x-postion in Au
    initial_y_pos = orbit_0[2][idx] + (
        ((homeplanet_radius / Au) * np.cos((np.pi / 2) - np.pi / 2) * np.sin(phi))
    ...
    mission.set_launch_parameters(
        ...
        launch_position=[initial_x_pos[0], initial_y_pos[0]],
    ...
    pos_after_launch = spacecraft_triliteration(
        T + 446.7099999963486 / sec_per_year, distances
    )
